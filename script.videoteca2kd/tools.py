...
class Settings:
    def __init__(self, anime=False):
        ...
        self.url_address2 = self.settings.getSetting('url_address2')
        ...
        self.anime_folder = self.settings.getSetting('anime_folder')
        if self.anime_folder == '':  # define anime folder
            self.anime_folder = 'special://temp/animes/'
        # remove .strm
...
def translator(imdb_id, language):
    ...
    url_themoviedb = "http://api.themoviedb.org/3/find/%s?api_key=8d0e4dca86c779f4157fc2c469c372ca&language=%s" \
                     "&external_source=imdb_id" % (
                         imdb_id, language)
    response = browser1.get(url_themoviedb)
...
def goodSpider():
    ...
    import random
    sleep(random.randrange(100, 2000, 1) / 1000)
                ...
                if not os.path.isfile(fullFileName) or overwrite == 'true':
                    ...
                    if cont % 100 == 0 and time_noti > 0:
                        dialog.notification(name_provider, '%s %s encontrados - Trabajando...\n%s'
                                            % (cont, type_list, message), xbmcgui.NOTIFICATION_INFO, time_noti)
                    xbmc.log('[%s]%s Agregados' % (name_provider_clean, fullFileName))
                if not silence and pDialog.iscanceled(): break
            if not silence:
                pDialog.close()
            if cont > 0:
