...
class MetaData:
    ...
    def __init__(self, preparezip=False, tmdb_api_key='af95ef8a4fe1e697f86b8c194f2e5e11'):
        ...
        if DB == 'mysql':
            ...
            self.dbcon = database.connect(database=db_name, user=db_user, password=db_pass, host=db_address,
                                          buffered=True)
    ...
    def __get_tmdb_language(self):
        ...
        # else:
        #     return 'en'
        return ISO639_2.get(xbmc.getInfoLabel("System.Language"), "en")
    ...
    def _get_config(self, setting):
        ...
        if matchedrow:
            common.addon.log('Found config data in cache table for setting: %s value: %s' % (setting, dict(matchedrow)),
                             0)
    ...
    def insert_meta_installed(self, addon_id, last_update, movie_covers='false', tv_covers='false', tv_banners='false',
        ...
        try:
            self.dbcur.execute(sql_insert, (
                addon_id, movie_covers, tv_covers, tv_banners, movie_backdrops, tv_backdrops, last_update))
            self.dbcon.commit()
        except Exception, e:
            common.addon.log('************* Error inserting into cache db: %s' % e, 4)
            return

    def update_meta_installed(self, addon_id, movie_covers=False, tv_covers=False, tv_banners=False,
                              movie_backdrops=False, tv_backdrops=False, last_update=False):
        '''
        ...
        common.addon.log(
            'Updating addons table addon id: %s movie_covers: %s tv_covers: %s tv_banners: %s movie_backdrops: %s tv_backdrops: %s last_update: %s' % (
                addon_id, movie_covers, tv_covers, tv_banners, movie_backdrops, tv_backdrops, last_update), 2)
    ...
    def get_meta(self, media_type, name, imdb_id='', tmdb_id='', year='', overlay=6):
        ...
        common.addon.log('---------------------------------------------------------------------------------------', 2)
        common.addon.log(
            'Attempting to retreive meta data for %s: %s %s %s %s' % (media_type, name, year, imdb_id, tmdb_id), 2)
            ...
            # Else - they are online so piece together the full URL from TMDB
                ...
                if media_type == self.type_movie:
                    if meta['cover_url'] and len(meta['cover_url']) > 1 and not meta['cover_url'].startswith('http'):
                        meta['cover_url'] = self.tmdb_image_url + common.addon.get_setting('tmdb_poster_size') + meta[
                            'cover_url']
                    else:
                        meta['cover_url'] = ''
                    if meta['backdrop_url'] and len(meta['backdrop_url']) > 1 and not meta['backdrop_url'].startswith(
                            'http'):
                        meta['backdrop_url'] = self.tmdb_image_url + common.addon.get_setting('tmdb_backdrop_size') + \
                                               meta['backdrop_url']
                    else:
                        meta['backdrop_url'] = ''
    ...
    def update_meta(self, media_type, name, imdb_id, tmdb_id='', new_imdb_id='', new_tmdb_id='', year=''):
        ...
        '''
        common.addon.log('---------------------------------------------------------------------------------------', 2)
        common.addon.log('Updating meta data: %s Old: %s %s New: %s %s Year: %s' % (
            name.encode('ascii', 'replace'), imdb_id, tmdb_id, new_imdb_id, new_tmdb_id, year), 2)
    ...
    def _cache_lookup_by_name(self, media_type, name, year=''):
        ...
        if year != '':
            sql_select = sql_select + " AND year = %s" % year  # Always check with year
        common.addon.log('SQL Select: %s' % sql_select, 0)
            ...
            for meta in meta_group:
                ...
                if media_type == self.type_movie:
                    ...
                    self.dbcur.execute(sql_insert, (meta['imdb_id'], meta['tmdb_id'], meta['title'],
                                                    meta['year'], meta['director'], meta['writer'], meta['tagline'],
                                                    meta['cast'],
                                                    meta['rating'], meta['votes'], meta['duration'], meta['plot'],
                                                    meta['mpaa'],
                                                    meta['premiered'], meta['genre'], meta['studio'], meta['thumb_url'],
                                                    meta['cover_url'],
                                                    meta['trailer_url'], meta['backdrop_url'], meta['imgs_prepacked'],
                                                    meta['overlay']))
                    ...
                    self.dbcur.execute(sql_insert, (meta['imdb_id'], meta['tvdb_id'], meta['title'], meta['year'],
                                                    ...
                                                    meta['banner_url'],
                                                    meta['cover_url'], meta['trailer_url'], meta['backdrop_url'],
    ...
    def _get_tvdb_meta(self, imdb_id, name, year=''):
        ...
        if tvdb_id:
            ...
            if show is not None:
                ...
                if meta['plot'] == 'None' or meta['plot'] == '' or meta['plot'] == 'TBD' or meta[
                    'plot'] == 'No overview found.' or meta['rating'] == 0 or meta['duration'] == 0 or meta[
                    'cover_url'] == '':
                    common.addon.log(
                        ' Some info missing in TVdb for TVshow *** ' + name + ' ***. Will search imdb for more', 0)
                    tmdb = TMDB(api_key=self.tmdb_api_key, lang=self.__get_tmdb_language())
        ...
        if meta:
            ...
            for movie in meta['results']:
                if movie['release_date']:
                    ...
                    year = None
                movie_list.append({'title': movie['title'], 'original_title': movie['original_title'], 'imdb_id': '',
                                   'tmdb_id': movie['id'], 'year': year})
        else:
            common.addon.log('No results found', 2)
            return None
    ...
    def get_episode_meta(self, tvshowtitle, imdb_id, season, episode, air_date='', episode_title='', overlay=''):
        ...
        common.addon.log('---------------------------------------------------------------------------------------', 2)
        common.addon.log(
            'Attempting to retreive episode meta data for: imdbid: %s season: %s episode: %s air_date: %s' % (
                imdb_id, season, episode, air_date), 2)
    ...
    def update_episode_meta(self, name, imdb_id, season, episode, tvdb_id='', new_imdb_id='', new_tvdb_id=''):
        ...
        common.addon.log('---------------------------------------------------------------------------------------', 2)
        common.addon.log(
            'Updating episode meta data: %s IMDB: %s SEASON: %s EPISODE: %s TVDB ID: %s NEW IMDB ID: %s NEW TVDB ID: %s' % (
                name, imdb_id, season, episode, tvdb_id, new_imdb_id, new_tvdb_id), 2)
    ...
    def _cache_lookup_episode(self, imdb_id, tvdb_id, season, episode, air_date=''):
        ...
        '''
        common.addon.log('Looking up episode data in cache db, imdb id: %s season: %s episode: %s air_date: %s' % (
            imdb_id, season, episode, air_date), 0)
    ...
    def _cache_delete_episode_meta(self, imdb_id, tvdb_id, name, season, episode, air_date=''):
        ...
        if imdb_id:
            sql_delete = "DELETE FROM episode_meta WHERE imdb_id = '%s' AND tvdb_id = '%s' AND season = %s" % (
                imdb_id, tvdb_id, season)
            if air_date:
                sql_delete = sql_delete + ' AND premiered = "%s"' % air_date
            else:
                sql_delete = sql_delete + ' AND episode = %s' % episode

        common.addon.log('Deleting table entry: IMDB: %s TVDB: %s Title: %s Season: %s Episode: %s ' % (
            imdb_id, tvdb_id, name, season, episode), 0)
        common.addon.log('SQL DELETE: %s' % sql_delete, 0)
        try:
            self.dbcur.execute(sql_delete)
        except Exception, e:
    ...
    def _cache_save_episode_meta(self, meta):
        ...
        '''
        if meta['imdb_id']:
            sql_select = 'SELECT * FROM episode_meta WHERE imdb_id = "%s" AND season = %s AND episode = %s AND premiered = "%s"' % (
                meta['imdb_id'], meta['season'], meta['episode'], meta['premiered'])
            sql_delete = 'DELETE FROM episode_meta WHERE imdb_id = "%s" AND season = %s AND episode = %s AND premiered = "%s"' % (
                meta['imdb_id'], meta['season'], meta['episode'], meta['premiered'])
        elif meta['tvdb_id']:
            sql_select = 'SELECT * FROM episode_meta WHERE tvdb_id = "%s" AND season = %s AND episode = %s AND premiered = "%s"' % (
                meta['tvdb_id'], meta['season'], meta['episode'], meta['premiered'])
            sql_delete = 'DELETE FROM episode_meta WHERE tvdb_id = "%s" AND season = %s AND episode = %s AND premiered = "%s"' % (
                meta['tvdb_id'], meta['season'], meta['episode'], meta['premiered'])
        else:
            sql_select = 'SELECT * FROM episode_meta WHERE title = "%s" AND season = %s AND episode = %s AND premiered = "%s"' % (
                self._clean_string(meta['title'].lower()), meta['season'], meta['episode'], meta['premiered'])
            sql_delete = 'DELETE FROM episode_meta WHERE title = "%s" AND season = %s AND episode = %s AND premiered = "%s"' % (
                self._clean_string(meta['title'].lower()), meta['season'], meta['episode'], meta['premiered'])
    ...
    def update_trailer(self, media_type, imdb_id, trailer, tmdb_id=''):
        ...
        common.addon.log('Updating trailer for type: %s, imdb id: %s, tmdb_id: %s, trailer: %s' % (
            media_type, imdb_id, tmdb_id, trailer), 0)
        common.addon.log('SQL UPDATE: %s' % sql_update, 0)
        try:
            self.dbcur.execute(sql_update)
            self.dbcon.commit()
    ...
    def change_watched(self, media_type, name, imdb_id, tmdb_id='', season='', episode='', year='', watched='',
        ...
        '''
        common.addon.log('---------------------------------------------------------------------------------------', 2)
        common.addon.log('Updating watched flag for: %s %s %s %s %s %s %s %s %s' % (
            media_type, name, imdb_id, tmdb_id, season, episode, year, watched, air_date), 2)
        ...
        if media_type in (self.type_movie, self.type_tvshow, self.type_season):
            if not watched:
                ...
                if watched == 6:
                    ...
                    watched = 6
            self._update_watched(imdb_id, media_type, watched, tmdb_id=tmdb_id, name=self._clean_string(name.lower()),
                                 year=year, season=season, tvdb_id=tvdb_id)
            ...
            if not watched:
                ...
                if watched == 6:
                    ...
                    watched = 6
            self._update_watched(imdb_id, media_type, watched, name=name, season=season, episode=episode,
                                 tvdb_id=tvdb_id, air_date=air_date)

    def _update_watched(self, imdb_id, media_type, new_value, tmdb_id='', name='', year='', season='', episode='',
                        tvdb_id='', air_date=''):
        '''
        ...
        if media_type == self.type_movie:
            ...
            sql_update = "UPDATE season_meta SET overlay = %s WHERE imdb_id = '%s' AND season = %s" % (
                new_value, imdb_id, season)
        elif media_type == self.type_episode:
            if imdb_id:
        ...
        common.addon.log('Updating watched status for type: %s, imdb id: %s, tmdb_id: %s, new value: %s' % (
            media_type, imdb_id, tmdb_id, new_value), 0)
        common.addon.log('SQL UPDATE: %s' % sql_update, 0)
        try:
            self.dbcur.execute(sql_update)
            self.dbcon.commit()
    ...
    def update_season(self, tvshowtitle, imdb_id, season):
        ...
        common.addon.log('---------------------------------------------------------------------------------------', 2)
        common.addon.log(
            'Updating season meta data: %s IMDB: %s TVDB ID: %s SEASON: %s' % (tvshowtitle, imdb_id, tvdb_id, season),
            2)
    ...
    def _cache_lookup_season(self, imdb_id, tvdb_id, season):
        ...
        common.addon.log(
            'Looking up season data in cache db, imdb id: %s tvdb_id: %s season: %s' % (imdb_id, tvdb_id, season), 0)

        if imdb_id:
            sql_select = "SELECT a.*, b.backdrop_url FROM season_meta a, tvshow_meta b WHERE a.imdb_id = '%s' AND season =%s and a.imdb_id=b.imdb_id and a.tvdb_id=b.tvdb_id" % (
                imdb_id, season)
        elif tvdb_id:
            sql_select = "SELECT a.*, b.backdrop_url FROM season_meta a, tvshow_meta b WHERE a.tvdb_id = '%s' AND season =%s  and a.imdb_id=b.imdb_id and a.tvdb_id=b.tvdb_id" % (
                tvdb_id, season)
        else:
            return None
    ...
    def _cache_delete_season_meta(self, imdb_id, tvdb_id, season):
        ...
        sql_delete = "DELETE FROM season_meta WHERE imdb_id = '%s' AND tvdb_id = '%s' and season = %s" % (
            imdb_id, tvdb_id, season)
    ...
    def get_batch_meta(self, media_type, batch_ids):
        ...
        if cache_meta:
            for m in new_batch_ids:
                try:
                    x = next((i for i, v in enumerate(cache_meta) if
                              v['imdb_id'] == m[0] or v['tmdb_id'] == m[1] or v['name'] == m[2]), None)
                except:
                    x = None
                if x is None:
